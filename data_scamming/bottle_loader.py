...
def load_round_data(round_num):
    ...
    """
    base_path = f"../2024_data_bottles/round-{round_num}-island-data-bottle/"
    ...
    for day in range(round_num-3, round_num):
        ...
        prices_df = pd.read_csv(f"{base_path}{price_file}", sep=';')
        prices_df['day'] = day

        if round_num == 2:
            prices_df['product'] = 'ORCHIDS'
            prices_df['mid_price'] = prices_df['ORCHIDS']
        prices_dfs.append(prices_df)
        
        # Load trades
        # trade_file = f"trades_round_{round_num}_day_{day}_nn.csv"
        # trades_df = pd.read_csv(f"{base_path}{trade_file}", sep=';')
        # trades_df['day'] = day
